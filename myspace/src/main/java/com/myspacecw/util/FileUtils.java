...
public class FileUtils {
	...
	public List<Map<String, Object>> parseInsertFileInfo(Board boardVO, 
			MultipartHttpServletRequest mpRequest) throws Exception{
		
		String ypath = "/resources/upload/file/";
		...
		*/
		...
		while(iterator.hasNext()) {
			...
			if(multipartFile.isEmpty() == false) {
				...
				String[] type =  storedFileName.split("\\.");
				file = new File(path + storedFileName);
				multipartFile.transferTo(file);
				listMap = new HashMap<String, Object>();
				listMap.put("no", bno);
				listMap.put("oriName", originalFileName);
				listMap.put("type", type[1]);
				listMap.put("path", ypath);
				listMap.put("name", storedFileName);
